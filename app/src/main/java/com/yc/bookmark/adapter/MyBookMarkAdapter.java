...
public class MyBookMarkAdapter extends YcMultiBaseAdapter<MyBookMarkNoSelectListBean> {
    ...
    @Override
    protected void convert(YcBaseViewHolder holder, MyBookMarkNoSelectListBean data, int position, int viewType) {
        if (viewType == 0) { //是书签
            if (isEdit) {
...
//                holder.addOnClickListener(R.id.tv_bookmark_folder);
    ...
    @Override
    protected int getViewType(int i, MyBookMarkNoSelectListBean myBookMarkNoSelectListBean) {
        String type = myBookMarkNoSelectListBean.getType();
        if ("url".equals(type)) {
            return 0;
        }
        return 1;
